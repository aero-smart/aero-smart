...
/// The analog pressure sensors use the voltage among 0.5 V to 4.5 V to represent the pressure from 0 Pa to its maximum range.
...
pub fn voltage_v_to_pressure_pa(
    ...
    if !(0.5..=4.5).contains(&voltage_v) {
        defmt::warn!(
            "Analog pressure sensor voltage out of range: {} V",
            voltage_v
        );
    }
...
pub fn option_arr_to_message(input: [Option<f32>; 4]) -> AnalogPressureSensorData {
