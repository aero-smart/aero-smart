      ...
      <div class="lg:col-span-5 flex flex-col gap-4 min-h-0 h-full">
        <div class="bg-white rounded-2xl p-4 shadow-sm border border-white flex flex-col gap-4 relative h-full">
          ...
          <div class="grid grid-cols-2 gap-3 content-start auto-rows-min">
            <div 
              v-for="(metric, idx) in telemetryData" 
              :key="metric.id" 
...
function sendQuery() {
  ...
  if (q.includes('电池') || q.includes('power') || q.includes('功耗')) {
    aiResult.value = '当前电池电压稳定，建议关注高负载区间的电流峰值，适当降低瞬时推力阶跃。'
  } else if (q.includes('振动') || q.includes('imu')) {
    aiResult.value = 'IMU 姿态波动在可控范围，若出现周期性尖峰，优先检查安装紧固与减震结构。'
  } else if (q.includes('压差') || q.includes('压力') || q.includes('气流')) {
    aiResult.value = '压差趋势平稳，建议核对管路密封与滤波窗口，避免低速段噪声放大。'
  } else {
    aiResult.value = '建议结合当前气流、压差与 IMU 姿态三者趋势进行交叉验证，优先排除传感器偏移。'
  }
}

function addSensor(id: string) {
  if (selectedSensorIds.value.includes(id)) return
  selectedSensorIds.value = [...selectedSensorIds.value, id]
}
...
function onDrop(targetId: string) {
  ...
  const ids = [...selectedSensorIds.value]
  const fromIndex = ids.indexOf(draggingId.value)
  const toIndex = ids.indexOf(targetId)
  if (fromIndex === -1 || toIndex === -1) return
  ids.splice(fromIndex, 1)
  ids.splice(toIndex, 0, draggingId.value)
  selectedSensorIds.value = ids
