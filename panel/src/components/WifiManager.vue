  ...
  <div class="bg-white rounded-2xl p-4 shadow-sm border border-white flex flex-col gap-4">
    <div class="flex justify-between items-center">
      ...
      <div v-if="status.connected" class="flex items-center gap-2">
        ...
        <button @click="handleDisconnect" :disabled="connecting" class="text-xs bg-red-50 text-red-600 px-2 py-1 rounded hover:bg-red-100">
          Disconnect
        </button>
      </div>
    ...
    <div v-if="networks.length > 0" class="flex flex-col gap-2 max-h-60 overflow-y-auto">
      ...
      >
        <div class="flex flex-col">
          ...
          <span class="text-[10px] text-gray-400">Signal: {{ net.signal }}% | {{ net.security }}</span>
        </div>
    ...
    <!-- Password Modal -->
    <div v-if="showPasswordModal" class="fixed inset-0 bg-black/20 flex items-center justify-center z-50">
